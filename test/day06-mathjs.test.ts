...
import * as day06_mathjs from "../src/day06-mathjs";
import {Sequence} from "../src/sequence";

describe.skip("Mathjs version is long-running", () => {
    describe("GridOfLights class", () => {
        it("Starts off all dark", () => {
            const lights = new day06_mathjs.GridOfLights();
    ...
    describe("#parseInstruction", () => {
        it("Splits instruction string into values", () => {
            expect(day06_mathjs.parseInstruction("turn on 0,0 through 999,999")).toStrictEqual(["turn on", 0,0, 999,999]);
            expect(day06_mathjs.parseInstruction("toggle 0,0 through 999,0")).toStrictEqual(["toggle", 0,0, 999,0]);
            expect(day06_mathjs.parseInstruction("turn off 499,499 through 500,500")).toStrictEqual(["turn off", 499,499, 500,500]);
        });

        it("Rejects unexpected strings", () => {
            expect(() => day06_mathjs.parseInstruction("")).toThrow();
            expect(() => day06_mathjs.parseInstruction("toggle a,b through 999,0")).toThrow();
            expect(() => day06_mathjs.parseInstruction("trouble 499,499 through 500,500")).toThrow();
            expect(() => day06_mathjs.parseInstruction("turn on 9999,499 through 500,500")).toThrow();
        });
    });
    ...
    describe("#litAfterInstructions", () => {
        it("Runs a sequence of instruction strings", async () => {
            ...
            expect(await day06_mathjs.litAfterInstructions(lines)).toBe(1_000_000 - 1_000 - 4);
        });
    });
    ...
    describe("GridOfLightsV2 class", () => {
        it("Starts off all dark", () => {
            const lights = new day06_mathjs.GridOfLightsV2();
    ...
    describe("#brightnessAfterInstructions", () => {
        it("Runs a sequence of instruction strings", async () => {
            ...
            expect(await day06_mathjs.brightnessAfterInstructions(lines)).toBe(1_000_000 + 2_000 - 4);
        });
    });
});